...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['bayar'])) {
    ...
    // Ambil stok sekarang
    $stokQuery = mysqli_query($conn, "SELECT stok FROM menu WHERE id='$id'");
    $stokData = mysqli_fetch_assoc($stokQuery);

    if ($stokData && $qty > 0 && $qty <= $stokData['stok']) {
        // Kurangi stok
        $update = mysqli_query($conn, "UPDATE menu SET stok = stok - $qty WHERE id='$id'");

        if ($update) {
            // Redirect agar halaman refresh dan update terlihat
            ...
            <div class="col-md-4 mb-3">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form onsubmit="event.preventDefault(); showQRModal('<?php echo htmlspecialchars($row['nama']); ?>', <?php echo $row['harga']; ?>, <?php echo $row['id']; ?>, this.qty.value)">
